...
def import_case(file_format: str, path):
    ...
    decision_makers_option_value = np.empty((df.decision_makers_option.nunique(), df.internal_variable_input.nunique()))
    for i in range(len(df)):
    ...
    input_model_objects = np.append(np_dict['fixed_inputs'], np.append(internal_variable_inputs, external_variable_inputs))
    ...
    for i in range(no_key_outputs):
        key_output_weight[i] = key_output_weight_unsorted[np.where(key_outputs_unsorted == np_dict['key_outputs'][i])[0][0]]
