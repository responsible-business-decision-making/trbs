    ...
    def make_slides_pdf(self, scenario, orientation):
        ...
        for input_tables in ["key_outputs_theme", "decision_makers_options", "scenarios"]:
            ...
            if image.shape[1] > max_image:
                ...
                pdf.image(str(self.random_number) + '/table' + input_tables + '.png', x=x_pos,
                          y=60)
            pdf.footer_page(self.name, orientation)
        ...
        for input_tables in ["fixed_inputs"]:
            ...
            for number_iteration in range(0, number_of_iterations):
                ...
                if number_of_iterations > 1:
                    pdf.chapter_title(self.make_title(input_tables) + ' ' + str(number_iteration + 1) + '/' + str(
                        number_of_iterations), rgb)
                else:
                    pdf.chapter_title(self.make_title(input_tables), rgb)
                pdf.chapter_subtitle(self.make_introduction(input_tables))
                ...
                max_image, width_image, x_pos = determine_position_images(orientation, image)
                if image.shape[1] > max_image:
                    pdf.image(str(self.random_number) + '/table' + input_tables + str(number_iteration) + '.png', x=25,
                              y=50, w=width_image)
                else:
                    pdf.image(str(self.random_number) + '/table' + input_tables + str(number_iteration) + '.png',
                              x=x_pos,
                              y=50)
                pdf.footer_page(self.name, orientation)

        pdf.add_page()
        pdf.chapter_title("The decision maker option '" + self.output_dict[scenario]['highest_weighted_dmo'] +
                          "' has the highest weighted appreciations for scenario: " + scenario, rgb)
